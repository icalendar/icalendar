...
describe Icalendar::Availability do
  ...
  describe "#append_custom_property" do
    ...
    context "with a defined property" do
      it "sets the proper setter" do
        subject.append_custom_property "summary", "availability"
        expect(subject.summary).to eq "availability"
  ...
  describe '.parse' do
    ...
    let(:fn) { 'availability.ics' }

    it 'should return an availabilitys array' do
      availabilitys = Icalendar::Availability.parse(source)
      expect(availabilitys).to be_instance_of Array
      expect(availabilitys.count).to be 1
      expect(availabilitys.first).to be_instance_of Icalendar::Availability
    end
  end
  ...
  describe '#to_ical' do
    before(:each) do
      ...
      subject.summary = 'My availability, my ical, my test'
    ...
    it { expect(subject.to_ical).to include 'SUMMARY:My availability\, my ical\, my test' }
